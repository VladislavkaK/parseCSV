...
try {
    ...
    $db->exec("TRUNCATE collections_films");

    $stmt = $db->prepare("INSERT INTO collections_films (film_id,id, name) " .
        "SELECT :id,(:name::JSON->>'id')::INT8,:name::JSON->>'name' AS collections");
    ...
    while (($row = fgetcsv($file)) !== false) {
        ...
        try {
            $film = FilmCollections::fromCSV($headers, $row);

            $id = $film->getId();
            $stmt->bindValue(':id', $id);
            $name = $film->getCollections();
            ...
            $name = preg_replace("/, \"poster_path.*/", '}', $name);
