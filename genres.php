...
try {
    ...
    $line = 0;
    $count = 0;
    $db->exec("TRUNCATE genres_films");

    $stmt = $db->prepare("INSERT INTO genres_films (film_id,id, name) " .
        "SELECT :id,(json_array_elements(:name)->>'id')::INT8,json_array_elements(:name)->>'name' AS genres");
    ...
    while (($row = fgetcsv($file)) !== false) {
        ...
        try {
            $film = FilmGenres::fromCSV($headers, $row);
